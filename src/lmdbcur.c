               ...
               MDB_cursor_op cop)
{
    ...
    // BEWARE we are temporarily pointing to data the caller owns; fixed below.
    // Note that LMDB requires casting away const, but doesn't mutate.
    self->mkey.mv_data = (void*)key;
    self->mkey.mv_size = key_size;

    // We don't have to set mval to NULL to assist clients, as we're
    // filling it just below
    ...
    // The caller may have supplied a cop that gets the values as well as
...
lmdbcur_new_fromkey (lmdbdbi_t *dbi, lmdbtxn_t *txn,
                     const void *key, size_t key_size)
{
    return s_new_withcop (dbi, txn, key, key_size, MDB_SET);
}
