/*  =========================================================================
    classlmdb_selftest.c - run selftests
...
// Tests for draft public classes:
    { "lmdbenv", lmdbenv_test },
    { "lmdbdbi", lmdbdbi_test },
    { "lmdbtxn", lmdbtxn_test },
    { "lmdbcur", lmdbcur_test },
#endif // CLASSLMDB_BUILD_DRAFT_API
#ifdef CLASSLMDB_BUILD_DRAFT_API
    { "private_classes", classlmdb_private_selftest },
#endif // CLASSLMDB_BUILD_DRAFT_API
    {0, 0}          //  Sentinel
};
...
{
    test_item_t *item;
    printf ("Running classlmdb selftests...\n");
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("classlmdb_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running classlmdb test '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
